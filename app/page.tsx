...
/**
 * CALENDARIO APPESO - ARCHITETTURA TITANIO V7
 * ANALISI MENTORE:
 * 1. REGOLA 1 (STRICT PATHS): Percorso Firebase corretto /artifacts/{appId}/users/{userId}/data
 * 2. REGOLA 3 (AUTH FIRST): Accesso garantito prima di ogni query.
 * 3. FIX X ROSSA: Rimosso proxy locale inesistente. Uso di AllOrigins con timeout e validazione ICS.
 * 4. ANTI-NAN: Ogni calcolo di stile CSS è protetto da fallback numerici.
 */
...
const appId = (typeof __app_id !== 'undefined' ? __app_id : 'family-calendar-pro').replace(/\//g, '_');
...
const Utils = {
  ...
  getSafeTimeParts: (t: any): [number, number] => {
    if (typeof t !== 'string' || !t.includes(':')) return [0, 0];
    const p = t.split(':');
      ...
      return (
        <div className="h-screen flex items-center justify-center p-10 bg-slate-50 text-center font-sans">
          <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border border-red-50 max-w-sm">
            <Icons.Logo />
            <h2 className="text-2xl font-black text-slate-800 mt-6 mb-2 tracking-tighter uppercase">Riavvio App</h2>
            <p className="text-slate-500 text-xs mb-8">Un errore di calcolo ha bloccato il sistema. Ripristino in corso.</p>
            <button onClick={() => window.location.reload()} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg">Ripristina</button>
          </div>
        </div>
      );
    }
...
function AppContent() {
  ...
  // 2. Data Listener (REGOLA 1: Strict Path)
  useEffect(() => {
    if (!user) return;
    // Percorso blindato: /artifacts/{appId}/users/{userId}/data/config
  ...
  const save = async (evs: any[], srcs: any[]) => {
    if (!user) return;
    try {
  ...
  // 3. iCal Sync Logic - RESILIENTE
  useEffect(() => {
    const srcs = (icalSources || []).filter(s => s && s.url && s.url.startsWith('http'));
    ...
    const fetchAll = async () => {
      ...
      for (const s of srcs) {
        ...
        try {
          // Utilizziamo un proxy pubblico affidabile per l'anteprima
  ...
  const parseICS = (text: string) => {
    ...
    blocks.forEach(block => {
      try {
        const summary = block.match(/SUMMARY:(.*)/)?.[1]?.trim() || "Google Event";
        ...
        const d = new Date(
          ...
          parseInt(s.substring(9,11)) || 0, 
          parseInt(s.substring(11,13)) || 0
        );
  ...
  const allEventsCombined = useMemo(() => {
    const local = Array.isArray(events) ? events : [];
    const external = Array.isArray(icalEvents) ? icalEvents : [];
    return [...local, ...external].filter(e => e && e.date);
  }, [events, icalEvents]);
  ...
  if (initializing) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-900 text-white">
      <Icons.Loading />
      <span className="text-[10px] font-black uppercase tracking-[0.3em] mt-4 opacity-50">Sincronizzazione Cloud...</span>
    </div>
  );
  ...
  return (
    <div className="flex h-screen w-full bg-slate-100 font-sans text-slate-900 overflow-hidden">
      ...
      <aside className="w-80 bg-slate-900 text-white p-10 flex flex-col hidden lg:flex shadow-2xl z-20">
        ...
        <div className="bg-gradient-to-br from-white/10 to-transparent p-8 rounded-[2.5rem] border border-white/10 backdrop-blur-md mb-10 group">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar space-y-8">
          <div>
            ...
            <div className="space-y-4">
              {(Array.isArray(icalSources) ? icalSources : []).map((s, i) => (
                <div key={i} className="group relative bg-white/5 p-5 rounded-[2rem] border border-white/5 hover:bg-white/10 transition-all">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-black truncate w-40">{s.name || 'Sorgente Sync'}</span>
                    {icalStatuses[s.url] === 'loading' && <Icons.Loading />}
                    {icalStatuses[s.url] === 'success' && <Icons.Check />}
                    {icalStatuses[s.url] === 'error' && <Icons.Error />}
                  </div>
                  <div className="text-[9px] text-slate-500 truncate opacity-40">{s.url}</div>
                  <button onClick={() => { const n = icalSources.filter((_, idx) => idx !== i); setIcalSources(n); save(events, n); }} className="absolute -right-2 -top-2 bg-red-500 p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition shadow-xl active:scale-90"><Icons.X /></button>
                </div>
              ))}
              <button onClick={() => setIsSettingsOpen(true)} className="w-full py-4 rounded-[1.8rem] border-2 border-dashed border-white/5 text-slate-600 text-[10px] font-black uppercase tracking-widest hover:border-blue-500 hover:text-blue-500 transition-all">+ Nuova Sorgente</button>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50">
        <header className="bg-white border-b border-slate-200 px-8 py-6 flex justify-between items-center z-10 shadow-sm">
          <div className="flex items-center gap-6">
            ...
            <div className="flex bg-slate-100 rounded-2xl p-1.5">
              <button onClick={() => { const d = new Date(currentDate); d.setMonth(d.getMonth() - 1); setCurrentDate(d); }} className="p-2 hover:bg-white rounded-xl transition"><Icons.ChevronLeft /></button>
              ...
              <button onClick={() => { const d = new Date(currentDate); d.setMonth(d.getMonth() + 1); setCurrentDate(d); }} className="p-2 hover:bg-white rounded-xl transition"><Icons.ChevronRight /></button>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-auto bg-white relative no-scrollbar">
          {view === 'month' ? (
            <MonthView date={currentDate} events={allEventsCombined} onDayClick={(d:string) => { setSelectedDate(d); setIsModalOpen(true); }} onDelete={(id:string) => {
              const next = events.filter(e => e.id !== id); setEvents(next); save(next, icalSources);
            }} />
          ) : (
            <WeekView date={currentDate} events={allEventsCombined} onTimeClick={(d:string) => { setSelectedDate(d); setIsModalOpen(true); }} onDelete={(id:string) => {
              const next = events.filter(e => e.id !== id); setEvents(next); save(next, icalSources);
            }} />
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-sm shadow-2xl border border-white/20 animate-in zoom-in duration-200">
             <div className="flex justify-between items-center mb-8">
                <h3 className="text-2xl font-black uppercase tracking-tighter">Turno<br/><span className="text-blue-600 text-[10px] tracking-widest uppercase mt-1 inline-block">{selectedDate}</span></h3>
                <button onClick={() => setIsModalOpen(false)} className="p-2 text-slate-300 hover:text-slate-900 transition"><Icons.X /></button>
             </div>
             <div className="space-y-6">
                <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Titolo turno..." className="w-full bg-slate-50 p-5 rounded-3xl font-black text-sm outline-none border-none shadow-inner" />
                <div className="flex gap-4">
                  <input type="time" value={start} onChange={e => setStart(e.target.value)} className="flex-1 bg-slate-50 p-4 rounded-2xl font-black text-center shadow-inner" />
                  <input type="time" value={end} onChange={e => setEnd(e.target.value)} className="flex-1 bg-slate-50 p-4 rounded-2xl font-black text-center shadow-inner" />
                </div>
                <button onClick={addShift} className="w-full bg-blue-600 py-6 rounded-[2.5rem] text-white font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">Salva Dashboard</button>
             </div>
          </div>
        </div>
      )}
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3.5rem] p-12 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-6">
              <input id="ical-url-field" placeholder="https://link-del-calendario.ics" className="w-full bg-slate-50 p-6 rounded-[2rem] font-bold text-xs mb-4 outline-none shadow-inner" />
              <button onClick={() => {
                const url = (document.getElementById('ical-url-field') as HTMLInputElement).value;
                if (!url || !url.startsWith('http')) return;
                const next = [...icalSources, { url, name: 'Sorgente Google' }];
                setIcalSources(next); save(events, next); setIsSettingsOpen(false);
              }} className="w-full bg-slate-900 text-white py-6 rounded-[2.5rem] font-black uppercase tracking-widest text-xs active:scale-95 transition-all shadow-xl">Attiva Sync Ora</button>
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-7 auto-rows-fr bg-slate-100 gap-[1px]">
        {days.map((d, i) => {
          ...
          const ds = Utils.fmtDate(d), dayEvts = (events || []).filter((e:any) => e && e.date === ds), isToday = ds === Utils.fmtDate(new Date());
          return (
            <div key={ds} onClick={() => onDayClick(ds)} className={`bg-white p-4 hover:bg-blue-50/30 transition-colors cursor-pointer group min-h-[140px] ${isToday ? 'ring-2 ring-inset ring-blue-500 z-10 shadow-inner' : ''}`}>
              ...
              <div className="space-y-1.5 overflow-hidden">
                {dayEvts.map((e:any) => (
                  <div key={e.id} className={`${e.color} text-[9px] p-2.5 rounded-2xl font-black truncate relative shadow-sm border border-black/5`}>
                    {e.startTime} {e.title}
...
function WeekView({ date, events, onTimeClick, onDelete }: any) {
  ...
  const weekDays = Array.from({length: 7}).map((_, i) => Utils.addDays(start, i));
  const ROW_HEIGHT = 75;
    ...
    <div className="h-full flex flex-col min-w-[900px]">
      ...
      <div className="flex-1 relative" style={{ height: `${24 * ROW_HEIGHT}px` }}>
        <div className="absolute inset-0 flex">
          ...
          {weekDays.map(d => {
            const ds = Utils.fmtDate(d), dayEvts = (events || []).filter((e:any) => e && e.date === ds);
            return (
